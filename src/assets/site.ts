...
function initScramble(): void {
  ...
  const arm = (pointerX: number, pointerY: number) => {
    ...
    if (deadline.size && !frame) frame = requestAnimationFrame(resolve);
  };
...
function initYearFloat(): void {
  ...
  const update = () => {
    ...
    for (const tile of tiles) {
      if (tile.getBoundingClientRect().bottom >= probe) { year = tile.dataset.year ?? ''; break; }
    }
...
function initZoom(): void {
  ...
  const apply = (unit: string, persist: boolean) => {
    ...
    if (persist) { try { localStorage.setItem(KEY, unit); } catch { /* private mode */ } }
  };

  for (const step of steps) step.addEventListener('click', () => apply(step.dataset.unit ?? '126', true));
  try {
...
function initLightbox(): void {
  ...
  const fill = (tile: HTMLElement, index: number, total: number) => {
    ...
    if (picture) {
      ...
      for (const source of clone.querySelectorAll('source')) source.setAttribute('sizes', '(max-width: 900px) 92vw, 46vw');
  ...
  box.addEventListener('touchend', (e) => {
    ...
    if (Math.abs(dx) > 48 && Math.abs(dx) > Math.abs(dy)) show(current + (dx < 0 ? 1 : -1), true);
  }, { passive: true });
...
function initPanels(): void {
  ...
  const triggers = document.querySelectorAll<HTMLAnchorElement>('[data-exhibition], [data-panel-open]');
...
function initReveals(): void {
  ...
  window.setTimeout(() => {
    if (revealed === 0) { targets.forEach((el) => el.classList.add('is-in')); observer.disconnect(); return; }
